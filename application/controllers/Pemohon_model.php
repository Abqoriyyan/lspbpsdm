...
class Pemohon_model extends CI_Model
{
    ...
    public function list_permohonan_skk($nik){
        ...
        JOIN ( SELECT * FROM history_permohonan WHERE LOG IN (SELECT MAX(LOG) FROM history_permohonan WHERE kode_status NOT IN('90', '50') GROUP BY id_izin)) b ON b.id_izin = a.id_izin 
        JOIN data_klasifikasi_kualifikasi_permohonan c ON c.id_izin = a.id_izin 
        JOIN master_kualifikasi d ON c.kualifikasi = d.id 
        WHERE a.nik = '$nik' GROUP BY id_izin";    
